    ...
    <nav className="bg-white py-6 shadow-md fixed top-0 w-full z-50"> {/* Fixed navbar with white background */}
      <div className="container mx-auto flex justify-between items-center relative">
        ...
        <ul className="flex space-x-12 text-black text-lg font-mono font-semibold">
          {['Shop', 'Accessories', 'About', 'Retailers', 'Media'].map((item) => (
            <li key={item} className="relative group cursor-pointer transition-colors duration-300">
              <span className="hover:text-gray-700 transition-colors duration-300">{item}</span>
              <span className="absolute bottom-0 left-0 w-0 h-1 bg-red-500 transition-all duration-500 group-hover:w-full"></span> {/* Red underline */}
            </li>
          ))}
        </ul>
